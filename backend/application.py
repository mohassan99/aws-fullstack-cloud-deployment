...
def fetch_data_from_db():
    ...
    """
    Fetch all events ordered by date ascending and return JSON-ready rows.
    """
    create_db_table()
    ...
    try:
        ...
        for row in rows:
            ...
            if row_date is not None:
                dt = datetime.combine(row_date, time(0, 0), tzinfo=timezone.utc)
                formatted_date = format_datetime(dt, usegmt=True)
            else:
                formatted_date = None
